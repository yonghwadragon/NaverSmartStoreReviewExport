...
- iframe 자동 탐지 & 리뷰 수집
- 리뷰탭 진입 후, 리뷰 영역 자동 스크롤 (사람처럼 아래로 내리기)
"""
...
# ============================================================
# 8) 리뷰 영역 자동 스크롤 (사람이 내리는 것처럼)
# ============================================================
def smooth_scroll(target, steps: int = 10, delay: float = 0.3):
    """
    target: Page 또는 Frame
    window.scrollBy 로 아래로 여러 번 스크롤
    """
    try:
        for _ in range(steps):
            target.evaluate("window.scrollBy(0, 800)")
            time.sleep(delay)
    except Exception:
        # 스크롤 실패해도 전체 로직은 계속 가도록
        pass
...
def scrape_reviews(url: str, limit_pages: int, cookie_data: dict):
    ...
    with sync_playwright() as p:
        ...
        for n in range(1, limit_pages + 1):
            # ✅ 리뷰 페이지마다, 먼저 스크롤을 충분히 내려줌
            smooth_scroll(iframe, steps=10, delay=0.25)
